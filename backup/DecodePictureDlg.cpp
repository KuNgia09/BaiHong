...
BOOL Task(char* input,char* output)
{
	...
	//获取原始加密文件大小和FileKey
	int offset=dwFileSize-8;
	
	SetFilePointer(hFile,offset,0,FILE_BEGIN);

	
	pFileKey=(DWORD*)malloc(8);
	memset(pFileKey,0,8);
	ReadFile(hFile,pFileKey,8,&dwActual,0);

	int FileKey=*pFileKey;
	int unSize=*(pFileKey+1);
	
	if(FileKey!=0x12345678)
	{
		OutputDebugString("不是加密文件");
		goto result;
	}
	sprintf(szInfo,"file key is %x,size is %d",FileKey,unSize);
	OutputDebugString(szInfo);
	

	//此时dwFileSize是真正的加密数据大小
	dwFileSize=dwFileSize-8;
	SetFilePointer(hFile,0,0,FILE_BEGIN);
	pBuffer=(unsigned char*)malloc(dwFileSize);
	ReadFile(hFile,pBuffer,dwFileSize,&dwActual,NULL);
	...
	sprintf(szInfo,"dwFileSize:%d,unsize:%d",dwFileSize,unSize);
	OutputDebugString(szInfo);
